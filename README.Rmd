...
1. **Correctly Specified Model:** The expected proportions and effective sample size ($N_{eff}$) match the underlying data-generating process.
2. **Misspecified $N_{eff}$:** The model assumes double the true effective sample size ($N_{eff} = 50$ vs. true $N_{eff} = 25$), overestimating data precision.
3. **Misspecified Selectivity:** The model assumes logistic selectivity, but the true underlying process follows dome-shaped selectivity.

We calculate the OSA residuals with `run_osa()` and compare the diagnostic output using `plot_osa()`.
To ensure bubble sizes remain visually consistent across different stock assessments and fleet comparisons, `plot_osa()` caps maximum bubble sizes at 6 and issues a warning listing any truncated values:

```{r example,  fig.width = 10, fig.height = 8, fig.keep='last'}
...
Trijoulet, V., Nielsen, A. 2022. [_compResidual: Residual calculation for compositional observations._](https://github.com/fishfollower/compResidual) R package version 0.0.1.
